...
async function runJourney(
  ...
  // 3. 角色頁：「iPhone 已連接，角色狀態已同步」＋成員清單用手機的名字。
  card = await openSyncCard(page, narrow);
  await expect(card.getByText("iPhone 已連接，角色狀態已同步")).toBeVisible({ timeout: 20_000 });
  ...
  // 5. 斷線：畫面說「iPhone 暫時離線」，不是「沒有裝置」也不是「已同步」。
  ...
  await waitCharacterSession(
    request,
    (payload) => memberPresence(payload, phone.deviceId) === "offline",
    20_000,
    target()
  );
  await expect(card.getByText("iPhone 暫時離線")).toBeVisible({ timeout: 30_000 });
  await expect(card.locator(".badge-ok")).toHaveCount(0);
  await page.screenshot({ path: path.join(OUT, `${options.shot}-offline.png`) });
  ...
  await expect(card.getByText("iPhone 已連接，角色狀態已同步")).toBeVisible({ timeout: 30_000 });
...
test("角色同步：鍵盤到得了五個主入口與同步卡；減少動態時卡片仍讀得到", async ({ page }) => {
  ...
  await page.reload();
  const card = await openSyncCard(page, false);
  await expect(card.getByText(/尚未連接 iPhone|iPhone|角色同步目前關閉/).first()).toBeVisible({
    timeout: 20_000,
  });
