...
打開桌面控制中心就夠了：首次啟動有**三步設定精靈**（之後可略過的「先試一次」體驗），
接著只有五個入口：「現在 → 角色（預設顯示目前角色的名字，例如小樞）→ 工作 →
連接與權限 → 更多」，每一頁都是人話；待決定的事在右上角收件匣。
...
| 角色 | Character（Character Adapter／manifest） | 桌面上陪你的角色；可更換、匯入或接上外部引擎，但它沒有任何權限，只會「演出」Runtime 給它的狀態 |
| 角色說「做完了」／「已確認完成」 | claimed-completed／verified | 前者只是 Agent 的說法，後者是你親自確認過；綠勾只在後者出現 |
