...
> 一般／進階模式的切換在「更多 → 進階功能」。
>
> 本文描述的是 `main` 上 v0.5 Phase 8（2026-09-02）的控制中心。文中引用的 `assets/v05-evidence/` 截圖是 Phase 7
> 證據跑產生的（五入口已在、但角色頁／工作頁／連接頁的 Phase 8 文案與版面尚未套用）；Phase 8 的新截圖會在下一次
> 證據跑（`pnpm test:e2e` 的 evidence spec）後落到同一個資料夾。v0.2–v0.4 的圖片一律視為升級歷史，集中在文末。
...
「角色離線，改用文字。」；「現在」頁與角色頁也會標示「角色目前無法顯示，改用文字」。安全訊息（緊急停止、被擋下、
結果不確定）在文字模式一樣會出現，不會因為沒有動畫就消失。
...
第一次啟動自動開啟（設定頁可隨時重來）。三步：**認識〔角色名〕 → AI 幫手 → 安全預設**。

- 只預選低風險、本機、不需硬體的能力；**攝影機／麥克風／位置／對外寫入永不預選**。
- AI 幫手一步只做 Codex／Claude Code 的安裝與登入狀態檢查、寫入 agent 分工路由，不授權任何工作區寫入。
...
側欄這一格顯示目前角色的名字（預設「小樞」；你改過名字就顯示你取的名字；角色還沒載入前顯示中性的「角色」）。頁內區塊依序：

| 區塊 | 內容 |
|---|---|
| 目前角色 | 名字、來源（內建／匯入／外部）、即時狀態（角色視窗運作中／已隱藏／準備中／角色視窗未連線／角色目前無法顯示，改用文字）、預覽（小樞是 36 表情預覽；sprite 角色是動作預覽；文字角色是文字範例） |
| 外觀與名字 | 取名字（≤24 字）、配色／變體、角色自己宣告的偏好（來自 manifest 的 `preferencesSchema`，只允許布林／數字／字串／列舉） |
| 平常如何陪伴 | 氣泡、音效（預設關）、拖曳、玩耍與互動、桌面移動、個性（安靜／自然／活潑）、說話風格（Persona）、重看角色劇情 |
| 安靜與勿擾 | 使用者層級勿擾開關（一小時內不要主動說話／今天安靜一點也在角色快捷選單） |
| 主動式對話 | 由本機 AI 幫手產生的主動訊息（每日產生次數上限、用哪個幫手） |
| 主動程度與安靜時段 | AI 主動程度、安靜時段——**唯一的編輯處**；同意與安全頁只顯示摘要 |
| 更換或加入角色 | 角色卡片（內建／匯入；標示 第三方／外部／需要網路／有可執行程式）、選用、停用、移除、**匯入角色** |

**匯入角色**：貼上角色描述檔（manifest）並提供它宣告的資產。匯入只做驗證與存檔，**不會執行任何程式、不連線、不下載**；
...
- 輸入框：任務描述（第一行會變成工作的標題）、**加入檔案或選擇資料夾**（路徑文字欄；「選擇資料夾…」按鈕只在桌面版出現，
  且這個版本尚未內建資料夾選擇器，會誠實回報 這個版本沒有資料夾選擇器）、**這是哪一種工作**（寫程式／對話／整理知識／審查）。
- **開始前預覽**（固定六項，永遠先看再開始）：使用哪個 Agent／讀取範圍／是否寫入／工具／時間、訊息與費用上限／如何取消。
  預設唯讀、30 分鐘 TTL、費用上限 0.5 USD；勾選寫入後還要第二次確認，而且只對明確的資料夾有效。
...
| 區塊 | 內容 | 按鈕 |
|---|---|---|
| 可以看見 | 已啟用的感知來源（人話名稱、影響徽章） | 管理感知來源 |
| 可以回應 | 已啟用的回應方式 | 管理回應方式／工具操作 |
| 使用的裝置 | 已連接的裝置與來源（六階：只發現／已配對／已安裝設定尚未連線／已連線尚未測試／已測試／已啟用）＋**角色如何接上系統** | 加入或掃描裝置 |
| 需要你確認 | 待決定（同意、agent 等待）、目前授權、緊急停止中 → 前往解除 | 處理 |

四區下方是展開狀態的「**全部能力與裝置**」，裡面就是完整的能力中心（感知來源／回應方式／工具操作／裝置與提供者／硬體掃描／iPhone 配對），
每張卡片都有誠實的確認層級（「可以確認：通知已送達作業系統／無法確認：你是否已經看見」）與「提供者尚未提供完整的資料與影響說明」的保守提示。

**角色如何接上系統**（在「使用的裝置」內）：每個角色 adapter 一列——內建或第三方、本機或外部、是否有可執行程式、是否需要網路、
可以接收哪些資料、是否已測試、連線中與否；外部 adapter 可以在這裡**撤銷**（立即斷線，不會自己回來）。
桌面角色的呈現層 provider 也列在完整能力中心裡，並註明「這是桌面角色的呈現層：只負責演出，不持有任何權限。」

> 目前的限制：外部 adapter 的「可以接收」與「有可執行程式／需要網路」在 instance 清單還沒帶 `inputCapabilities`／
> `securityRequirements` 之前顯示「未回報」——這是介面拿不到資料時的誠實文案，不是裝置真的沒有能力。

**同意與安全**：使用授權（能力選擇器授予／撤回，撤回後明確顯示「新動作已阻止；進行中的已要求取消」）、風險分級 L0–L4 的人話說明、
安全規則摘要（編輯在角色頁）、**緊急停止解除流程**（顯示停止原因與時間、列出「會恢復／不會自動恢復」的能力清單、再次確認；
高風險與需同意能力永不自動恢復）。緊急停止由右上角紅鈕**二段確認**觸發，觸發後右上角變成「緊急停止中 — 前往解除」。

![連接與權限（v0.5 Phase 7 證據截圖；Phase 8 改為四區塊版面）](assets/v05-evidence/desktop-connect.png)
...
| 分頁 | 內容 |
|---|---|
| 記憶與知識 | 一般模式只有三區：關於我的記憶／〔角色名〕學會的知識／素材與來源；可執行更新決策、送出使用者糾正、複審。使用者糾正不會直接變成普遍知識 |
| 活動歷史 | 人話的活動故事，嚴格區分 已規劃／已授權／已排入／已送出／已收到／說做完了／驗證成功／被擋下／已取消／結果不確定；原始 receipt 只在進階 |
| 設定 | 語言／外觀／縮放／Reduced Motion、視窗與啟動（關閉行為、登入時啟動、啟動時顯示角色／開啟控制中心）、重新執行首次設定、備份入口、版本（技術資料折疊） |
| 角色與整合管理 | 通往角色頁與「連接與權限 → 使用的裝置」的整理入口 |
| 進階功能 | **「顯示進階功能」開關的唯一主人** |
