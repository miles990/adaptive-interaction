...
> v0.5 的一級導覽簡化為 **5 個入口：現在、小樞、工作、連接與權限、更多**。
> 工作＝AI 工作階段＋自動互動；連接與權限＝裝置與能力＋同意與安全（含權限地圖）；
> 更多＝記憶與知識＋活動歷史＋設定。待決定事項走右上角 Inbox。
> 每項設定只有一個主人：小樞的外觀、主動對話、主動程度與安靜時段都在「小樞」頁。
> 下方的 v0.2/v0.3/v0.4 圖片與頁名是升級歷史，v0.4 closing 畫面見
> [`assets/v04-evidence/`](assets/v04-evidence/)，v0.5 畫面見
> [`assets/v05-evidence/`](assets/v05-evidence/)。
...
v0.5 起預設是 **shu-maid**（Q 版貓娘女僕 rig；classic／dusk／sakura 三種配色），舊的
v1/v2 sprite 角色包保留為相容層；說話風格（Persona）可換世界觀
（例如「導航員」把「做完了」講成「任務節點已完成」），但**安全訊息永遠是固定的
標準文字，任何角色包都無法覆寫或隱藏**。外觀、名字、玩耍與互動開關全部只在
「小樞」頁調整（設定頁只留摘要與「前往小樞」）。
...
第一次啟動自動開啟（設定頁可隨時重來）。v0.5 起縮為三步：
**認識小樞 → AI 幫手 → 安全預設**。低風險本機能力自動保守挑選（未知不選）；
AI 幫手一步只做 Codex／Claude Code 的安裝與登入狀態檢查，不授權任何工作區寫入；
硬體掃描、iPhone 配對、安靜時段細調移到第一次真正需要時再詢問。
...
> 以下「一般模式各頁」是 v0.4 的 9 頁說明，保留作為各區塊功能的參考；v0.5 起它們
> 折疊進 5 個入口（現在＝首頁；工作＝AI 工作階段＋自動互動；連接與權限＝感知來源／
> 回應方式／工具操作＋同意與安全；更多＝記憶與知識＋活動歷史＋設定），舊 tab id
> 仍可深連結。

### 首頁（v0.5：現在）
系統是否正常、主動互動狀態（含**暫停**控制）、感測／待決定／進行中工作摘要條、
最近一次互動的五步故事（感知→理解→計畫→安全→結果）、快速操作。
**三區權限地圖已移到「連接與權限 → 同意與安全」**（唯一的家），首頁不再重複。
...
- **安全規則**：AI 主動程度／安靜時段的**編輯**在「小樞」頁（單一主人）；本頁只顯示摘要與
  「前往小樞」，JSON merge patch 只在進階頁。
...
### 設定
一般／進階模式切換、語言／外觀／縮放／Reduced Motion、視窗與啟動（關閉行為、登入時啟動、
啟動時顯示小樞／開啟控制中心）、重新執行首次設定、備份入口、版本。小樞的外觀與互動
開關不在這裡（在「小樞」頁）。
