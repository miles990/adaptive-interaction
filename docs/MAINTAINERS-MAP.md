...
| **已知限制** | ESP32 真板驗收為零（只有 `compile.sh` 編譯檢查與 pty 模擬器）；MQTT／BLE 共用程式碼但沒有 AIP session 測試；參考韌體 639 bytes 單行上限讓部分回覆送不出去（稽核 `aip.outbound-undeliverable`）。成員 `syncProfile` 與裝置線 v1.2 分片**本輪在另一個 worktree 進行中，合併後補 file:line** |
