...
| 第二種裝置（宣告式 Serial）經正式 AIP binding 成為 session 成員 | 達成（**pty 模擬器經 production `DeviceLink`＋serial adapter**，非真板；v0.6.x `e86afb9`） | `crates/interaction-runtime/tests/declarative_session_loop.rs` 7 測（加入／touch 讓 revision 前進／declare＋SensorSource／stop-all 真 ack／靜默＝unknown／撤銷不影響其他成員／拔線 reconnecting→offline）＋`aip_link.rs` 6＋`esp32_sim_conformance.rs` 三方一致 2；核心零變更由獨立驗證者以 `git diff --stat` 核對。已知：snapshot 回覆放不進 639 bytes 單行（稽核 `aip.outbound-undeliverable`）；韌體只編譯檢查，**ESP32 真板為零**；MQTT／BLE 未測 |
