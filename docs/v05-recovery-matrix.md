...
- **§18-19／§6.4** Migration 與向後相容 → 存在：crates/interaction-storage/src/lib.rs:58-62 `store.migrate()`；`apps/interaction-desktop/src/routing.ts` 的 `LEGACY_ANCHORS`（v0.6.x 前在 `App.tsx:64-79`）把舊 tab id 折疊到新 5 入口（CHANGELOG Phase 1 宣稱相容）；v1/v2 sprite pack fallback 有測試（p2-rig）。LEGACY_ANCHORS 對應表的單元測試自 v0.6.x 起在 `src/test/deep-links.test.tsx`（13 個舊錨點逐字釘住＋27 個可路由 id）；HomePage.tsx:134 仍導向孤立 legacy tab `responses`（p1-ia 已列）
