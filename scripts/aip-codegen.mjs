...
import { semanticArtifacts } from "./semantic-state-codegen.mjs";
...
function generateSwiftFixtures() {
  const names = readdirSync(FIXTURES_DIR)
    ...
    .sort();
  const released = "releases/v0.7.0";
  for (const name of readdirSync(join(FIXTURES_DIR, released)).filter((n) => n.endsWith(".json")).sort()) names.push(`${released}/${name}`);
  const out = [];
  out.push("// GENERATED by scripts/aip-codegen.mjs — do not edit.");
